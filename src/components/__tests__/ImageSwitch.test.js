...
describe("ImagesSwitch", () => {
    it("renders correctly with default props", () => {
        const { getByLabelText } = render(<ImagesSwitch numberOfImages={1} onImagesChange={() => {}} />);
        const switchElement = getByLabelText("1 Image");
        expect(switchElement).toBeInTheDocument();
    });
 
    it("renders correctly with 4 images", () => {
        const { getByLabelText } = render(<ImagesSwitch numberOfImages={4} onImagesChange={() => {}} />);
    ...
    it("calls onImagesChange with 1 when switch is toggled from 4 to 1", () => {
        ...
        const { getByRole } = render(<ImagesSwitch numberOfImages={4} onImagesChange={onImagesChange} />);
    ...
    it("calls onImagesChange with 4 when switch is toggled from 1 to 4", () => {
        const onImagesChange = jest.fn();
        const { getByRole } = render(<ImagesSwitch numberOfImages={1} onImagesChange={onImagesChange} />);
        const switchElement = getByRole("checkbox");
        fireEvent.click(switchElement);
